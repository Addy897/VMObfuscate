...
void VM::vm_print() {
  ...
  int i = 0;
#ifdef _WIN32
  // printf args are pushed rcx, rdx, r8,r9
  int rcx = tokens[TOKENS_TYPE::RCX];
  arg1 = vars[rcx].get_str();
  args[i++] = tokens[TOKENS_TYPE::RDX];
  args[i++] = tokens[TOKENS_TYPE::R8];
  args[i++] = tokens[TOKENS_TYPE::R9];
  int args_len = 3;
#elif defined(__linux__)
  // printf args are  pushed in this order rdi,rsi,rdx,rcx,r8,r9
  int rdi = tokens[TOKENS_TYPE::RDI];
  arg1 = vars[rdi].get_str();
  args[i++] = tokens[TOKENS_TYPE::RSI];
  args[i++] = tokens[TOKENS_TYPE::RDX];
  args[i++] = tokens[TOKENS_TYPE::RCX];
  args[i++] = tokens[TOKENS_TYPE::R8];
  args[i++] = tokens[TOKENS_TYPE::R9];
  int args_len = 5;
#endif
...
void VM::handle_internal_function(INTERNAL_FUNCTION func) {
  ...
  case INTERNAL_FUNCTION::CALL_MessageBoxA: {
    ...
    MessageBoxA((HWND)(uintptr_t)arg1, arg2.c_str(), arg3.c_str(), arg4);
    vars.erase(rcx);
    vars.erase(rdx);
    vars.erase(r8);
    vars.erase(r9);
...
void VM::fromhex(string &hexstr, string &result) {
  if (hexstr.rfind("0x", 0) != string::npos)
...
void VM::start_runtime() { run_function("main"); }
